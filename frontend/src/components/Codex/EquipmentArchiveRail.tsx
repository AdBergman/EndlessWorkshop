import type {
    ActiveEquipmentArchiveFilters,
    EquipmentArchiveFilterGroup,
    EquipmentArchiveFilterKey,
} from "@/lib/codex/codexEquipmentArchiveFilters";

type Props = {
    activeFilters: ActiveEquipmentArchiveFilters;
    groups: readonly EquipmentArchiveFilterGroup[];
    onClearFilters: () => void;
    onToggleFilter: (filterKey: EquipmentArchiveFilterKey, value: string) => void;
};
...
export default function EquipmentArchiveRail({
    activeFilters,
    groups,
    onClearFilters,
    onToggleFilter,
}: Props) {
    const hasActiveFilters = Boolean(activeFilters.type || activeFilters.rarity);

    return (
        <div className="codex-resultsFilters" aria-label="Equipment filters">
