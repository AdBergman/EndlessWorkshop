...
class DistrictControllerTest {
    ...
    void getAllDistricts_returnsJson() throws Exception {
        ...
        mockMvc.perform(get("/api/districts")
                ...
                .andExpect(jsonPath("$[0].constructibleLevel").doesNotExist())
                .andExpect(jsonPath("$[0].constructionCost.length()").value(0))
                ...
                .andExpect(jsonPath("$[0].levelUp.validNeighbourDescriptorKeys.length()").value(0))
                .andExpect(jsonPath("$[0].levelUp.validNeighbourUiMapperKey").doesNotExist())
