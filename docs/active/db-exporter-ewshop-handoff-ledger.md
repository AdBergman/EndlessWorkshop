...
- Direction: EWShop internal implementation
- Topic: sequential Codex data-path audit across export, import, persistence,
  API, frontend stores, and Codex enrichment.
- Summary: EWShop fixed proven contract losses in the rich Tech and Improvement
  paths while preserving the canonical `DBX-CODEX-REFERENCES-001` exporter
  request generated by the reference-integrity diagnostics refresh.
- Source docs/files:
  - `docs/active/codex-audit/category-audits.md`
  - `docs/active/final-snapshot-import-hygiene-audit.md`
  - `docs/active/codex-audit/exporter-request-register.md`
- Snapshot/export version: local `0.82` Codex/rich exports revalidated on
  2026-08-19.
- Decision/result: Improvement `category` is now preserved by the frontend
  contract. Rich Tech `technologyPrerequisiteTechKeys` and
  `exclusiveTechnologyPrerequisiteTechKeys` are now preserved from import DTO
  through persistence, `/api/techs`, frontend store, and Codex Tech enrichment.
  Existing production/common migrations already created
  `tech_prereq_key` and `tech_exclusive_prereq_key` in `V3_2_3`; `V3_5_6`
  hardens their FK/index behavior.
- EWShop implementation result: focused backend/API/frontend regression tests
  cover Tech prerequisite arrays and Improvement category preservation, and a
  migration-backed Flyway/JDBC test validates the existing prerequisite-key
  tables without relying on Hibernate schema generation.
- Follow-up generated: none; unresolved public reference classification remains
  tracked by the existing `DBX-CODEX-REFERENCES-001` entry.
- Status: EWShop implementation complete.
