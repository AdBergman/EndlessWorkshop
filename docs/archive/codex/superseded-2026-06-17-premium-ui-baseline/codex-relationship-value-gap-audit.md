...
| 5 | Thin Actions | What does this action do and when should I use it? | 139 Actions; 84 have only facts and no public description/mechanics section. | DB exporter/editorial | Keep exact-link/search targets; avoid promoting thin Actions as rich browse content. |
| 6 | Action cost/mechanic context | Why is this cost modifier relevant? | 55 Actions have sections, mostly cost modifiers or formula notes. | Both | Exporter summaries first; EWShop can later group/highlight useful mechanics. |
...
Current data shape: 26 Population entries; 74 threshold items; 21/21 exact unique threshold refs resolve; 0 exact refs are unresolved; 53 text-only threshold rewards.
...
Blocked examples:
- Aspect (Population_Aspect) keeps "Nutrient Extractor" as plain text and no longer emits the non-public Aspect_DistrictImprovement_00 ref.
...
DB exporter/editorial request: add exact threshold reward refs only where current public Codex targets exist; keep text-only rewards plain otherwise.
...
Current data shape: 139 Action entries; 55 have sections; 84 are facts-only; no Actions have descriptionLines in the current import.
...
Current data shape:
- Actions: 84/139 thin (Army Steal Territory (ActionTypeArmyStealTerritory)<br>Boost Cultural Economic Gain (ActionTypeBoostCulturalEconomicGain)<br>Buy Neighbour Settlement With Money (ActionTypeBuyNeighbourSettlementWithMoney)).
...
- Abilities: 18/336 thin (Blossom I (UnitAbility_Blossom_1)<br>Blossom II (UnitAbility_Blossom_2)<br>Deft Accessorizer (UnitAbility_Hero_Aspect03)).
