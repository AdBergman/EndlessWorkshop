...
Created: 2026-06-13
Source backlog: `docs/active/codex-category-jira-backlog.md`
...
| CDEX-CAT-004 | Establish Resource Codex Entities And Extractor Refs | Backend/product/exporter mixed | P0 | exporter packet now | High value, but Resource visibility/entity model needs product/backend decision before final implementation. | Yes, packet `CDEX-EXP-003`, blocked by product decision. | Product/backend must choose Resource export surface. |
...
- Readiness: blocked by product decision
...
- `resources` exportKind: product/backend decision required.
...
- Do not export Resource category until product/backend agrees it is public.
- Do not treat icons/tokens as entities.
- Do not expose hidden/unreleased resources.

### Open Questions

- Should Resource entries be top-level browseable, nested under Extractors, or
