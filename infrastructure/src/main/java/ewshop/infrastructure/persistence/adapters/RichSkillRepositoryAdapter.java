...
import ewshop.infrastructure.persistence.mappers.RichSkillMapper;
...
@Repository
public class RichSkillRepositoryAdapter implements RichSkillRepository {
    ...
    private final RichSkillMapper mapper;

    public RichSkillRepositoryAdapter(
    ...
    public RichSkills findAll() {
        return new RichSkills(
    ...
    @Override
    @Transactional
    public ImportResult importSkillSnapshot(RichSkillImportSnapshot snapshot) {
    ...
    private void importTrees(List<RichSkillImportSnapshot.SkillTreeSnapshot> snapshots, ImportResult result) {
        List<String> keepKeys = snapshots.stream().map(RichSkillImportSnapshot.SkillTreeSnapshot::treeKey).distinct().toList();
        ...
        for (RichSkillImportSnapshot.SkillTreeSnapshot snapshot : snapshots) {
    ...
    private void importTiers(List<RichSkillImportSnapshot.SkillTierSnapshot> snapshots, ImportResult result) {
        List<String> keepKeys = snapshots.stream().map(RichSkillImportSnapshot.SkillTierSnapshot::tierPlacementKey).distinct().toList();
        ...
        for (RichSkillImportSnapshot.SkillTierSnapshot snapshot : snapshots) {
    ...
    private void importSkills(List<RichSkillImportSnapshot.HeroSkillSnapshot> snapshots, ImportResult result) {
        List<String> keepKeys = snapshots.stream().map(RichSkillImportSnapshot.HeroSkillSnapshot::skillKey).distinct().toList();
        ...
        for (RichSkillImportSnapshot.HeroSkillSnapshot snapshot : snapshots) {
    ...
    private void importDefaults(List<RichSkillImportSnapshot.HeroSkillDefaultSnapshot> snapshots, ImportResult result) {
        if (snapshots.isEmpty()) return;
        List<String> keepKeys = snapshots.stream().map(RichSkillImportSnapshot.HeroSkillDefaultSnapshot::heroKey).distinct().toList();
        ...
        for (RichSkillImportSnapshot.HeroSkillDefaultSnapshot snapshot : snapshots) {
    ...
    private static boolean applySkill(
            ...
            RichSkillImportSnapshot.HeroSkillSnapshot snapshot,
            boolean isInsert
    ) {
        boolean changed = isInsert;
